...
function mqttsFactory(iface, fallback, mosca) {
  ...
  var wrap = buildWrap(mosca), options;
  if (iface.requestCert) {
    options = {
      rejectUnauthorized: iface.rejectUnauthorized || false,
      requestCert: iface.requestCert || false,
      caPath: credentials.caPath,
      SNICallback: credentials.SNICallback,
    }
  };
  var server = mqtt.createSecureServer(credentials.keyPath, credentials.certPath, wrap, options);
  return server;
}
